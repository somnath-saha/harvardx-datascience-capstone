...
# The following code is made availavle by the course to fetch the data from the grouplens website and extract a working dataset (edx) and a final validation dataset (validation) out of the dataset.
...
edx %>% group_by(title) %>%
   ...
   slice_max(n, n=10) %>%
   select("Movie Name" = title, "No. of Ratings" = n, "Avg Rating" = mu) %>% knitr::kable(caption = "Top 10 movie titles based on number of ratings")
...
# The edx dataset is divided into a training set and test set to develop and 
# test different models. 90% of the data as the training set is used to train the models 
# and these models are tested on the remaining 10% of the test dataset. 
# The model with the best prediction (lowest RMSE) on the test set is used as final model 
# to predict ratings for the validation set.
...
MovieRecommenderModel <- function(predictInputSet)
{
   ...
   predictInputSet <- predictInputSet %>% 
      semi_join(edx_train, by = "movieId") %>%
      semi_join(edx_train, by = "userId")
   ...
   RMSE(predictInputSet$rating, predicted_ratings)
}

final_rmse_validation_set <- MovieRecommenderModel(validation)
